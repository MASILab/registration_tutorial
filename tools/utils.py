...
def get_registration_command(registration_method_name, reg_args, reg_tool_root, fixed_image, moving_image, output_image, output_mat):
    ...
    if registration_method_name == 'affine_flirt':
        ...
        # reg_resample_path = os.path.join(reg_tool_root, 'reg_resample')
        # output_mat_real = output_mat.replace('.nii.gz', '.txt')
        # output_intensity_clipped = output_image.replace('/reg/', '/temp/').replace('.nii.gz', '_intens_clip_affine.nii.gz')
        # preprocessed_image_without_intens_clip = output_image.replace('/reg/', '/temp/sub_1k/')
        # output_affine_real = output_image
        # command_list.append(f'{reg_aladin_path} -ln 5 -ref {fixed_image} -flo {moving_image} -res {output_intensity_clipped} -aff {output_mat_real}')
        # command_list.append(f'{reg_resample_path} -inter 0 -pad -1000 -ref {fixed_image} -flo {preprocessed_image_without_intens_clip} -trans {output_mat_real} -res {output_affine_real}')
        reg_aladin_path = os.path.join(reg_tool_root, 'reg_aladin')
        # reg_resample_path = os.path.join(reg_tool_root, 'reg_resample')
        output_mat_real = output_mat.replace('.nii.gz', '.txt')
        # output_intensity_clipped = output_image.replace('/reg/', '/temp/').replace('.nii.gz', '_intens_clip_affine.nii.gz')
