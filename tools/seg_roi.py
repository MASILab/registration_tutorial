...
def run_preprocess_method(method_str, nii_file_path, mask_file_path, roi_region_path, output_file_path, fsl_root):
    if method_str == 'lung_seg_roi_clip_full':
        run_roi_clip(nii_file_path, mask_file_path, roi_region_path, output_file_path)
    elif method_str == 'lung_seg_roi_z_only':
        run_roi_clip_v2(nii_file_path, mask_file_path, roi_region_path, output_file_path, fsl_root)
...
def run_roi_clip(ori_path, mask_path, roi_region_path, roi_clip_path):

    get_mask_image(ori_path, mask_path)
    xmin, xsize, ymin, ysize, zmin, zsize = get_roi(mask_path, roi_region_path)
    clip_roi_region(ori_path, roi_clip_path, xmin, xsize, ymin, ysize, zmin, zsize)
...
def clip_roi_region(ori_img_path, roi_img_path, xmin, xsize, ymin, ysize, zmin, zsize, fsl_root):
    t0 = time.time()
    ...
    print('Clipping image %s...' % ori_img_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fsl_root', type=str)
    args = parser.parse_args()

    # run_roi_clip(args.ori, args.mask, args.roi_region, args.roi)
    run_preprocess_method(args.method, args.ori, args.mask, args.roi_region, args.roi, args.fsl_root)
