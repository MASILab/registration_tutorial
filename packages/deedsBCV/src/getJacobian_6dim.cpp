...
int main (int argc, char * const argv[]) {
    ...
    if(argc<4||argv[1][1]=='h'){
        ...
        cout<<"./getJacobian -M moving.nii.gz -O output -J J_det.nii.gz\n";
        cout<<"=============================================================\n";
        return 1;
    }
    ...
    size_t split_def=args.jacobian_file.find_last_of("/\\");
    ...
    if(args.jacobian_file.substr(args.jacobian_file.length()-2)!="gz"){
    ...
    cout<< "Calculate Jacobian determinant map, save into " << args.jacobian_file.substr(split_def+1)<<"\n";
//    cout<<"Transforming "<<args.moving_file.substr(split_moving+1)<<" into "<<args.jacobian_file.substr(split_def+1)<<"\n";
    ...
    // A the field of determinant of Jacobian.
    // It is in the space of original space, but we can transform it to the target space using the transformation field.
    float* J_det=new float[sz];
    fill(J_det, J_det+sz, (float)0);
//    jacobian_determinant(J_det, ux, vx, wx, m, n, o);
    jacobian_determinant_log(J_det, ux, vx, wx, m, n, o);

    gzWriteNifti(args.jacobian_file,J_det,header,m,n,o,1);
